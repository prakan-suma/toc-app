...
const PaginationContent = ({ data }) => {
  ...
  return (
    <div className="flex flex-col justify-between mt-2 items-center gap-4">
      <ShowContent data={paginatedData} />
      <div className="flex items-center gap-5">
        ...
        <button className="text-white bg-[#d2caff] px-2 p-1 rounded-md hover:bg-[#cac4d0]" onClick={handlePrevious} disabled={currentPage === 0}>
          <p className="font-semibold">Previous</p>
        </button>
        <span className="text-black font-semibold px-2">
          Page {currentPage + 1} of {totalPage}
        </span>
        <button className="text-white bg-[#d2caff] px-2 p-1 rounded-md hover:bg-[#cac4d0]" onClick={handleNext} disabled={currentPage === totalPage - 1}>
